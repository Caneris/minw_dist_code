...
if __name__ == '__main__':
    ...
    min_w_par_arr = np.array([1e-14, 0.2])
    ...
    csv_names = np.array(['lambda_LM', 'min_w_par'])
    # par_vals_list = [lambda_LM_arr, min_w_par_arr, sigma_w_arr, sigma_m_arr, N_app_arr, N_good_arr]
    par_vals_list = [lambda_LM_arr, min_w_par_arr]
    for gamma_s_i in range(len(gamma_s_arr)):
        ...
        for i in range(len(par_vals_list)):
            ...
            for parval in parvals:
                ...
                with open('dist_sim_results/dist_sim_qvals_{}_{}.csv'.format(csv_name, gamma_s_i), 'a', newline = '') as csvfile:
                    filewriter = csv.writer(csvfile, delimiter = ',')
                    filewriter.writerows(diff)
                for j in range(results2.shape[0]):
                    with open('dist_sim_results/dist_sim_mean_data_{}_{}_{}.csv'.format(csv_name, j, gamma_s_i), 'a', newline = '') as csvfile:
                        filewriter = csv.writer(csvfile, delimiter = ',')
